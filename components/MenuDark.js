...
function MenuDark() {
  ...
  const toggleMenu = () => {
    if (isMenuOpen) {
      ...
      // Устанавливаем задержку для полного закрытия меню
      setTimeout(() => {
        setIsMenuOpen(false);
        setIsClosing(false);
      }, 500); // 500 мс соответствует длительности анимации в CSS
    } else {
      setIsMenuOpen(true);
    }
  };
  ...
  // const toggleMenu = () => {
  //   if (isMenuOpen) {
  //     // Закрытие с задержкой для плавности
  //     setIsMenuOpen(false);
  //     setTimeout(() => setIsMenuOpen(false), 500); // Установите таймер равным времени анимации
  //   } else {
  //     // Открытие меню
  //     setIsMenuOpen(true);
  //   }
